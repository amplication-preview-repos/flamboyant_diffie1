...
const service = {
  ...
  clickEvents: () => FIND_MANY_RESULT,
  clickEvent: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("ClickEvent", () => {
  ...
  test("GET /clickEvents/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/clickEvents"}/${nonExistingId}`)
  ...
  test("GET /clickEvents/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/clickEvents"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /clickEvents existing resource", async () => {
